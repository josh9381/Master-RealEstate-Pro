...
export const createCustomFieldSchema = z.object({
  ...
  type: z.string().transform(v => v.toUpperCase()).pipe(z.enum(fieldTypes, { message: `Type must be one of: ${fieldTypes.join(', ')}` })),
...
export const updateCustomFieldSchema = z.object({
  ...
  fieldKey: z.string().min(1).max(100).regex(/^[a-z0-9_]+$/, 'Field key must be lowercase alphanumeric with underscores').optional(),
  type: z.string().transform(v => v.toUpperCase()).pipe(z.enum(fieldTypes)).optional(),
