...
export const listCampaignsQuerySchema = z.object({
  ...
  status: z.string().optional().transform(val => {
    ...
    // Support comma-separated statuses (e.g., "SCHEDULED,PAUSED")
    const allowed = ['DRAFT', 'SCHEDULED', 'ACTIVE', 'SENDING', 'PAUSED', 'COMPLETED', 'CANCELLED'];
    const statuses = val.split(',').map(s => s.trim()).filter(s => allowed.includes(s));
