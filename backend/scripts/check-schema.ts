...
async function main() {
  ...
  const dbTables: Array<{ table_name: string }> = await p.$queryRaw`SELECT table_name FROM information_schema.columns WHERE column_name = 'organizationId' ORDER BY table_name`;
  ...
  const fks: Array<{ table_name: string; constraint_name: string; delete_rule: string }> = await p.$queryRaw`
