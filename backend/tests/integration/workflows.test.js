...
describe('Workflow System Integration Tests', () => {
    ...
    describe('Workflow Trigger Detection', () => {
        test('Should trigger workflow on lead creation', async () => {
            ...
            const leadResponse = await (0, supertest_1.default)(server_1.default)
                .post('/api/leads')
                .set('Authorization', `Bearer ${authToken}`)
                .send({
            ...
            const executionsResponse = await (0, supertest_1.default)(server_1.default)
        ...
        test('Should NOT trigger inactive workflows', async () => {
            ...
            await (0, supertest_1.default)(server_1.default)
                ...
                .set('Authorization', `Bearer ${authToken}`)
                .send({ isActive: false });
            const previousExecutionsResponse = await (0, supertest_1.default)(server_1.default)
                .get(`/api/workflows/${testWorkflowId}/executions`)
                .set('Authorization', `Bearer ${authToken}`);
    ...
    describe('Workflow with Conditions', () => {
        test('Should trigger workflow only when conditions match', async () => {
            ...
            const workflowResponse = await (0, supertest_1.default)(server_1.default)
                .post('/api/workflows')
                .set('Authorization', `Bearer ${authToken}`)
                .send({
            ...
            const workflowCheckResponse = await (0, supertest_1.default)(server_1.default)
    ...
    describe('Workflow Actions', () => {
        ...
        test('Should execute CREATE_TASK action', async () => {
            ...
            const tasksResponse = await (0, supertest_1.default)(server_1.default)
                .get(`/api/tasks`)
                .set('Authorization', `Bearer ${authToken}`)
                .query({ leadId: testLeadId });
            expect(tasksResponse.status).toBe(200);
            // Should have task created by workflow
            const workflowTask = tasksResponse.body.data.tasks.find((t) => t.leadId === testLeadId);
