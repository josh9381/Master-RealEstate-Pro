...
require("./setup"); // Import test setup to configure environment
const supertest_1 = __importDefault(require("supertest"));
const express_1 = __importDefault(require("express"));
const database_1 = require("../src/config/database");
const bcryptjs_1 = __importDefault(require("bcryptjs"));
const jsonwebtoken_1 = __importDefault(require("jsonwebtoken"));
const activity_routes_1 = __importDefault(require("../src/routes/activity.routes"));
...
describe('Activity Management', () => {
    ...
    beforeEach(async () => {
        ...
        authToken = jsonwebtoken_1.default.sign({ userId: user.id, email: user.email, role: user.role }, process.env.JWT_ACCESS_SECRET || 'test-access-secret-123', {
            expiresIn: '24h',
    ...
    describe('GET /api/activities', () => {
        beforeEach(async () => {
            ...
            await database_1.prisma.activity.createMany({
                data: [
                    {
                        type: 'EMAIL_SENT',
                        title: 'Email 1',
        ...
        it('should filter activities by leadId', async () => {
            ...
            expect(response.body.data.activities.every((a) => a.leadId === leadId)).toBe(true);
        });
    ...
    describe('DELETE /api/activities/:id', () => {
        it('should delete an activity', async () => {
            ...
            expect(response.body.success).toBe(true);
            // Verify deletion
            const deleted = await database_1.prisma.activity.findUnique({
    ...
    describe('GET /api/activities/lead/:leadId', () => {
        it('should get all activities for a specific lead', async () => {
            // Create another lead
            const lead2 = await database_1.prisma.lead.create({
                data: {
                    name: 'Lead 2',
    ...
    describe('GET /api/activities/campaign/:campaignId', () => {
        it('should get all activities for a specific campaign', async () => {
            ...
            const campaign2 = await database_1.prisma.campaign.create({
                data: {
                    name: 'Campaign 2',
                    type: 'SMS',
                    status: 'DRAFT',
            ...
            expect(response.body.success).toBe(true);
            expect(response.body.data.activities).toHaveLength(2);
            expect(response.body.data.activities.every((a) => a.campaignId === campaignId)).toBe(true);
        });
