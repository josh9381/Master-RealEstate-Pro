...
const analytics_routes_1 = __importDefault(require("../src/routes/analytics.routes"));
    ...
    describe('GET /api/analytics/leads', () => {
        ...
        it('should return top leads', async () => {
            ...
            if (response.body.data.topLeads.length > 1) {
                ...
                expect(response.body.data.topLeads[0].score).toBeGreaterThanOrEqual(response.body.data.topLeads[1].score);
            }
        });
    });
...
async function createSampleData(userId) {
    ...
    // Create campaigns
    await Promise.all([
        database_1.prisma.campaign.create({
            data: {
